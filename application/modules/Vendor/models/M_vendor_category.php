<?php
class M_vendor_category extends CI_Model {
    private $primary_id='category_id';
    private $duplicate1='category_name';
    private $duplicate2='category_code';
    private $table_order='category_name';
    private $table_name='tb_vendor_category';
    private $site_id='';
	...
	function get_data_json_all($str='')
    {
        ...
        foreach($result->result() as $row)
        {
			...
			"<a  href='#modalForm' link-href='".base_url()."Vendor/Vendor_category/edit/".$row->category_id."' class='modal-with-zoom-anim on-default link-edit' title='Edit Row'><i class='fa fa-pencil'></i></a>
			<a  href='#modalConfirm' link-href='".base_url()."Vendor/Vendor_category/do_del/".$row->category_id."' class='modal-with-zoom-anim on-default link-del' data-del='".$row->category_name."'title='Delete Row'><i class='fa fa-trash-o'></i></a>"
			);
			$i++;
        }
        return $data;
