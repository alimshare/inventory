	...
	<header class="page-header">
		...
		<div class="right-wrapper pull-right">
			<ol class="breadcrumbs">
				...
				<li><span>Vendor Category</span></li>
            </ol>
		...
		<section class="panel">
			<header class="panel-heading" style=" border: 1px solid #5B707B; padding: 10px 14px; background: #5B707B;">
				<h2 class="panel-title" style="font-size: medium; color: white;"><i class="fa fa-chevron-right"> </i> Vendor Category Management</h2>
			</header>
				...
				<div class="row"> 
					...
					<div id="modalForm" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
						<form action="" id="addnew-form" method="post" class="form-horizontal mb-lg" >
							<section class="panel">
								<header class="panel-heading" style=" border: 1px solid #5B707B; padding: 10px 14px; background: #5B707B;">
									<h2 class="panel-title" style="font-size: medium; color: white;">Add New Category</h2>
								</header>
								<div class="panel-body">
...
(function($) {
	...
	var datatableInit = function() {
		...
		$('#addnew-form').submit(function(e) {
			...
			$.ajax({
				type : "POST",
				url  : '<?=base_url();?>Vendor/Vendor_category/do_add',
				data : data,
		...
		$("#datatable-ajax").on('click', '.link-edit', function(e) {
			...
			$.ajax({
				...
				success: function(response){
					...
					$("#txt_name").val(json_obj.category_name);
					$("#txt_code").val(json_obj.category_code);
				}
			});
		});
	...
	function LoadDatatable(){
		...
		var datatable = $table.dataTable({
			bProcessing: true,
			sAjaxSource: '<?=base_url();?>Vendor/Vendor_category/data_json_all',
			bDestroy: true,
			aoColumnDefs:[
			{
				bSortable: false,
	...
	function get_new_code(){
		$.ajax({
			type: 'GET',
			url: '<?php echo base_url();?>Vendor/Vendor_category/get_new_code',
			success: function(response) {
				if(response!=""){
