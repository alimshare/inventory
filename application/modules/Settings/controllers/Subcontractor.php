...
class Subcontractor extends CI_Controller {
	...
	private $page_titel='subcontractor';

	public function __construct()
	{
		parent::__construct();
		date_default_timezone_set('Asia/Jakarta');
		$this->load->model('m_subcontractor','mm');
	...
	public function index()
	{
		...
		$this->load->view('v_subcontractor.php');
	...
	public function do_add()
	{
		...
		if(isset($_POST['txt_id'])){
			...
			$data_update = array(
				...
				'subcontractor_address' => $address,
				'subcontractor_phone' 	=> $phone,
				'subcontractor_email' 	=> $email,
				'contract'				=> $contract,
			...
			if($this->is_unique_name($name)==FALSE){
				echo "The sub-contractor name already exists.";
			}else if($this->is_unique_code($code)==FALSE){
				echo "The sub-contractor code already exists.";
			}else{
				$data_insert = array(
					'subcontractor_code' 	=> $code,
					'subcontractor_name' 	=> $name,
	...
	public function get_new_code(){
		$data="001";
		$sql =  "SELECT DISTINCT subcontractor_code code FROM tb_subcontractor ORDER BY subcontractor_code DESC LIMIT 1 ";
...
/* Location: ./application/modules/settings/controllers/subcontractor.php */
