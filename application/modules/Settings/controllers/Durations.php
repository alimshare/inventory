...
class Durations extends CI_Controller {

	private $limit=20;
	private $site_id='';
	private $menu_titel='Settings';
	private $page_titel='Durations';
	...
	public function index()
	{
		...
		$this->load->view('v_durations.php');
		...
		if($this->is_unique_name($name)==FALSE){
			...
			if(isset($_POST['txt_id'])){
				...
				$data_update = array(
					'item_code' => $code,
					'item_name' => $name,
					'item_category' => 'duration',
					'modify_by' => $modify_by,
					'modify_date' => $modify_date
				);
				$data_update_id = $this->m_durations->do_update($id, $data_update);
			}else{
				$data_insert = array(
					'item_code' => $code,
					'item_name' => $name,
				...
				$data_insert_id = $this->m_durations->do_insert($data_insert);
			}
			echo "ok";
		}
	}
	...
	public function edit($id='')
	{
		...
		$data['data_all']=$this->m_durations->get_data_update($id);
	...
	{
		$data_update = $this->m_durations->do_delete($id);
		echo "ok";
	}
	...
	public function is_unique_name($str)
	{
		if(trim($str)==""){
			...
			$data_duplicate=$this->m_durations->get_data_duplicate_name($str);
			if(!isset($_POST['txt_id'])){
	...
	public function is_unique_code($str)
	{
		if(trim($str)==""){
			...
			$data_duplicate=$this->m_durations->get_data_duplicate_code($str);
			if(!isset($_POST['txt_id'])){
				if(count($data_duplicate)>0){
					return FALSE;
				}else{
