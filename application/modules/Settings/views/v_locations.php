	...
	<header class="page-header">
		...
		<div class="right-wrapper pull-right">
			<ol class="breadcrumbs">
				...
				<li><span>Project Locations</span></li>
            </ol>
		...
		<section class="panel">
			<header class="panel-heading" style=" border: 1px solid #5B707B; padding: 10px 14px; background: #5B707B;">
				<h2 class="panel-title" style="font-size: medium; color: white;"><i class="fa fa-chevron-right"> </i> Project Locations </h2>
			</header>
			<div class="panel-body">
				<div class="row"> 
					<div class="col-sm-6">
						<div class="mb-md">
							<a class="modal-with-zoom-anim btn btn-primary" href="#modalForm"><i class="fa fa-plus"></i> Add Location</a>
						</div>
					</div>
					...
					<div id="modalForm" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
						<form action="" id="addnew-form" method="post" class="form-horizontal mb-lg" >
							<section class="panel">
								<header class="panel-heading" style=" border: 1px solid #5B707B; padding: 10px 14px; background: #5B707B;">
									<h2 class="panel-title" style="font-size: medium; color: white;">Add Location</h2>
								</header>
								<div class="panel-body">
									<div class="form-group mt-lg">
										<label class="col-sm-3 control-label">Name of Project</label>
										<div class="col-sm-6">
									...
									<div class="form-group">
										...
										<div class="col-sm-6">
											...
											<select data-plugin-selectTwo class="form-control populate" id="txt_provinces_select" name="txt_provinces_select" style="width:100%">
												<option value="" title="">Select...</option>
												<?php if(isset($list_provinces) && count($list_provinces)>0){ ?>
												<?php foreach($list_provinces as $list){ ?>
									...
									<div class="form-group">
										...
										<div class="col-sm-6">
											<input type="text" name="txt_district" id="txt_district" class="form-control" value="" placeholder="Type district..." required />
										</div>
									</div>
									<!-- <div class="form-group">
...
(function($) {
	...
	var datatableInit = function() {
		...
		$('#addnew-form').submit(function(e) {
			...
			$.ajax({
				type : "POST",
				url  : '<?=base_url();?>Settings/Locations/do_add',
				data : data,
		...
		$("#datatable-ajax").on('click', '.link-edit', function(e) {
			...
			$.ajax({
				...
				success: function(response){
					...
					$("#edit-form").html("<input type='hidden' name='txt_id' id='txt_id' value='"+json_obj.loca_id+"'/>");
					$("#txt_name").val(json_obj.loca_name);
					$("#txt_code").val(json_obj.loca_code);
					$("#txt_district").val(json_obj.loca_district);
					$("#txt_provinces_select").val(json_obj.loca_province.toUpperCase().trim()).trigger('change');
	...
	function LoadDatatable(){
		...
		var datatable = $table.dataTable({
			bProcessing: true,
			sAjaxSource: '<?=base_url();?>Settings/Locations/data_json_all',
			bDestroy: true,
			aoColumnDefs:[
			{
				bSortable: false,
	...
	function get_new_code(){
		$.ajax({
			type: 'GET',
			url: '<?php echo base_url();?>Settings/Locations/get_new_location_code',
			success: function(response) {
				if(response!=""){
	...
	function LoadProvinces(province){
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url();?>Settings/Locations/get_provinces',
			data : 'province='+province,
			success: function(response) {
				$('#txt_provinces_select').html(response);
			}
		});
	}
