	...
	<header class="page-header">
		...
		<div class="right-wrapper pull-right">
			<ol class="breadcrumbs">
				...
				<li><span>Preferred Vendor</span></li>
            </ol>
		...
		<section class="panel">
			<header class="panel-heading" style=" border: 1px solid #5B707B; padding: 10px 14px; background: #5B707B;">
				<h2 class="panel-title" style="font-size: medium; color: white;"><i class="fa fa-chevron-right"> </i> Preferred Vendor</h2>
			</header>
				...
				<div class="row"> 
					...
					<div id="modalForm" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
						<form action="" id="addnew-form" method="post" class="form-horizontal mb-lg" >
							<section class="panel">
								<header class="panel-heading" style=" border: 1px solid #5B707B; padding: 10px 14px; background: #5B707B;">
									<h2 class="panel-title" style="font-size: medium; color: white;">Add New Vendor</h2>
								</header>
								<div class="panel-body">
									...
									<div class="form-group mt-lg">
										...
										<div class="col-sm-6">
											<select data-plugin-selectTwo class="form-control populate" id="txt_vendor_category" name="txt_vendor_category" style="width:100%">
												<option value="" title="">Select...</option>
												<?php foreach ($category as $key => $value): ?>
													<option value="<?php echo $value['category_id'] ?>" title=""><?php echo $value['category_code']." - ".$value['category_name'] ?></option>
												<?php endforeach ?>
											</select>
										</div>
									</div>
									...
									<div class="form-group">
										<label class="col-sm-3 control-label">PIC / Contact Persons Name</label>
										<div class="col-sm-6">
											<input type="text" name="txt_pic" id="txt_pic" class="form-control" value="" placeholder="Type PIC..."  />
										</div>
									</div>
									<div class="form-group">
									...
									<div class="form-group">
										<label class="col-sm-3 control-label">Vendor Since</label>
										<div class="col-sm-6">
											<input type="text" name="txt_since" id="txt_since" class="form-control" value="" data-plugin-datepicker />
										</div>
									</div>
...
(function($) {
	...
	var datatableInit = function() {
		...
		$('#addnew-form').submit(function(e) {
			...
			$.ajax({
				type : "POST",
				url  : '<?=base_url();?>Settings/Preferred_vendor/do_add',
				data : data,
		...
		$("#datatable-ajax").on('click', '.link-edit', function(e) {
			...
			$.ajax({
				...
				success: function(response){
					...
					$("#edit-form").html("<input type='hidden' name='txt_id' id='txt_id' value='"+json_obj.vendor_id+"'/>");
					$("#txt_name").val(json_obj.vendor_name);
					$("#txt_code").val(json_obj.vendor_code);
					$("#txt_address").val(json_obj.vendor_address);
					$("#txt_pic").val(json_obj.vendor_contact);
					$("#txt_phone").val(json_obj.vendor_phone);
					$("#txt_email").val(json_obj.vendor_email);
					$("#txt_since").val(json_obj.vendor_since);
					$("#txt_vendor_category").val(json_obj.category_id).trigger('change');
				}
			});
		});
	...
	function get_new_code(){
		$.ajax({
			type: 'GET',
			url: '<?php echo base_url();?>Settings/Preferred_vendor/get_new_code',
			success: function(response) {
				if(response!=""){
