	...
	<header class="page-header">
		...
		<section class="panel">
			<header class="panel-heading" style=" border: 1px solid #5B707B; padding: 10px 14px; background: #5B707B;">
				<h2 class="panel-title" style="font-size: medium; color: white;"><i class="fa fa-chevron-right"> </i> Consultant</h2>
			</header>
				...
				<div class="row"> 
					...
					<div id="modalForm" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
						<form action="" id="addnew-form" method="post" class="form-horizontal mb-lg" >
							<section class="panel">
								<header class="panel-heading" style=" border: 1px solid #5B707B; padding: 10px 14px; background: #5B707B;">
									<h2 class="panel-title" style="font-size: medium; color: white;">Add Consultant</h2>
								</header>
								<div class="panel-body">
...
(function($) {
	...
	var datatableInit = function() {
		...
		$('#addnew-form').submit(function(e) {
			...
			$.ajax({
				type : "POST",
				url  : '<?=base_url();?>Settings/Consultant/do_add',
				data : data,
		...
		$("#datatable-ajax").on('click', '.link-edit', function(e) {
			...
			$.ajax({
				...
				success: function(response){
					...
					$("#edit-form").html("<input type='hidden' name='txt_id' id='txt_id' value='"+json_obj.consultant_id+"'/>");
					$("#txt_name").val(json_obj.consultant_name);
					$("#txt_code").val(json_obj.consultant_code);
					$("#txt_contract").val(json_obj.contract);
					$("#txt_from").val(json_obj.term_of_service_from);
					$("#txt_to").val(json_obj.term_of_service_end);

					console.log(json_obj.project_id);
					$("#txt_project_select").val(json_obj.project_id).trigger('change');
					LoadUnit(json_obj.project_id, json_obj.unit_id);
					// $("#txt_unit_select").val(json_obj.unit_id).trigger('change');
					LoadLocation(json_obj.project_id, json_obj.loca_id);
					// $("#txt_location_select").val(json_obj.loca_id).trigger('change');
				}
			});
		});
	...
	function get_new_code(){
		$.ajax({
			type: 'GET',
			url: '<?php echo base_url();?>Settings/Consultant/get_new_code',
			success: function(response) {
				if(response!=""){
