	...
	<header class="page-header">
		...
		<div class="right-wrapper pull-right">
			<ol class="breadcrumbs">
				...
				<li><span>Unit/Department</span></li>
            </ol>
		...
		<section class="panel">
			<header class="panel-heading" style=" border: 1px solid #5B707B; padding: 10px 14px; background: #5B707B;">
				<h2 class="panel-title" style="font-size: medium; color: white;"><i class="fa fa-chevron-right"> </i> Unit Management</h2>
			</header>
				...
				<div class="row">
					...
					<div id="modalForm" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
						<form action="" id="addnew-form" method="post" class="form-horizontal mb-lg" >
							<section class="panel">
								<header class="panel-heading" style=" border: 1px solid #5B707B; padding: 10px 14px; background: #5B707B;">
									<h2 class="panel-title" style="font-size: medium; color: white;">Add New Unit</h2>
								</header>
								<div class="panel-body">
									<div class="form-group mt-lg">
										<label class="col-sm-3 control-label">Name of unit</label>
										<div class="col-sm-6">
...
(function($) {
	...
	var datatableInit = function() {
		...
		$('#addnew-form').submit(function(e) {
			...
			$.ajax({
				type : "POST",
				url  : '<?=base_url();?>Settings/Units/do_add',
				data : data,
		...
		$("#datatable-ajax").on('click', '.link-edit', function(e) {
			...
			$.ajax({
				...
				success: function(response){
					...
					$("#edit-form").html("<input type='hidden' name='txt_id' id='txt_id' value='"+json_obj.id+"'/>");
					$("#txt_name").val(json_obj.unit_name);
	...
	function LoadDatatable(){
		...
		var datatable = $table.dataTable({
			bProcessing: true,
			sAjaxSource: '<?=base_url();?>Settings/Units/data_json_all',
			bDestroy: true,
			aoColumnDefs:[
			{
				bSortable: false,
