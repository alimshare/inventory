	...
	<header class="page-header">
		...
		<section class="panel">
			<header class="panel-heading" style=" border: 1px solid #5B707B; padding: 10px 14px; background: #5B707B;">
				<h2 class="panel-title" style="font-size: medium; color: white;"><i class="fa fa-chevron-right"> </i> Consumable Stuff Classification</h2>
			</header>
							...
							<section class="panel">
								...
								<div class="panel-body">
									<div class="form-group mt-lg">
										...
										<div class="col-sm-4">
											<input type="text" name="txt_category" id="txt_category" class="form-control" value="" placeholder="Type Category..." required />
										</div>
									</div>
...
(function($) {
	...
	var datatableInit = function() {
		...
		$('#addnew-form').submit(function(e) {
			...
			$.ajax({
				type : "POST",
				url  : '<?=base_url();?>Settings/Consumable_stuff/do_add',
				data : data,
		...
		$("#datatable-ajax").on('click', '.link-edit', function(e) {
			...
			$.ajax({
				...
				success: function(response){
					...
					$("#txt_category").val(json_obj.item_category);
				}
			});
		});
	...
	function LoadDatatable(){
		...
		var datatable = $table.dataTable({
			bProcessing: true,
			sAjaxSource: '<?=base_url();?>Settings/Consumable_stuff/data_json_all',
			bDestroy: true,
			aoColumnDefs:[
			{
				bSortable: false,
