<?php
class M_consumable_stuff extends CI_Model {
    private $primary_id='item_id';
    private $duplicate1='item_name';
    private $duplicate2='item_code';
    private $duplicate3='item_category';
    private $table_order='item_name';
    private $table_name='tb_items';
    private $site_id='';
	...
	function get_data_json_all($str='')
    {
        ...
        foreach($result->result() as $row)
        {
			...
			"<a  href='#modalForm' link-href='".base_url()."Settings/Consumable_stuff/edit/".$row->item_id."' class='modal-with-zoom-anim on-default link-edit' title='Edit Row'><i class='fa fa-pencil'></i></a>
			<a  href='#modalConfirm' link-href='".base_url()."Settings/Consumable_stuff/do_del/".$row->item_id."' class='modal-with-zoom-anim on-default link-del' data-del='".$row->item_name."'title='Delete Row'><i class='fa fa-trash-o'></i></a>"
			);
			$i++;
        }
        return $data;
